...
def draw(X, fig, ax, MAX_NUM=20, threshold_1=1):
    '''
    MAX_NUM: 最大保留这么多圆
    ...
    # 绘制散点图
    # pic_total = ax.plot(X.real, X.imag, linestyle=':', color='gray')
    # X_fitted=np.fft.ifft(x) # 剔除小振幅后的图像效果，供测试用
    # ax.plot(X_fitted.real,X_fitted.imag)

    # 已经画出来的图像，后面填充
    pic_drawn = ax.plot([0], [0], color = "#99FFFF",linewidth = 3)
    # N-1个圆周,后面填充
    circle_plt_list = [ax.plot([0], [0], 'w')[0] for i in range(N)]

    # circle_plt_list = [ax.plot([0], [0], 'y')[0] if x_fo[i] != 0 else None
    #                    for i in range(n - 1)]
    # 直径线，后面填充
    radius_plt = ax.plot([0, 0], [0, 0], color='#DDDDDD', linestyle='-')
    ...
    def update_all(n):
        ...
        for j in range(N):
            ...
            circle_point = center + radius * np.exp(1j * np.linspace(0, 2 * np.pi, 50)) / N
